...
class InfGenerator:
    ...
    def generator(self, t:torch.Tensor) -> torch.Tensor:
        N = t.size(dim=2)
        if self.type == 'SE(2)':
            gen = self.sys.y(t[2]).reshape(N,1,1)*torch.tensor([[0., 0., 0.], [-1., 0., 0.], [0., 0., 0.]]).repeat(N,1,1).reshape(N,3,3) + \
                    self.sys.x(t[3]).reshape(N,1,1)*torch.tensor([[0., 0., 0.], [0., 0., 0.], [1., 0., 0.]]).repeat(N,1,1).reshape(N,3,3) + \
